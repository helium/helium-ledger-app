...
def _extend_and_serialize_multiple_derivations_paths(
        derivations_paths: List[bytes]):
...
class SolanaClient:
    ...
    def get_public_key(self, derivation_path: bytes) -> bytes:
        ...
        assert len(public_key.data
                   ) == PUBLIC_KEY_LENGTH, "'from' public key size incorrect"
        return public_key.data

    def sign_message(self, derivation_path: bytes, message: bytes,
                     accept: bool) -> RAPDU:
        ...
        if len(message_splited_prefixed) > 1:
            ...
            for m in message_splited_prefixed[:-1]:
                self._client.exchange(CLA, INS.INS_SIGN_MESSAGE, P1_CONFIRM,
                                      P2_MORE | P2_EXTEND, m)
        else:
            final_p2 = 0

        # response = self._exchange(INS_SIGN_MESSAGE, P1_CONFIRM, final_p2, message_splited_prefixed[-1])
        with self._client.exchange_async(CLA, INS.INS_SIGN_MESSAGE, P1_CONFIRM,
                                         final_p2,
